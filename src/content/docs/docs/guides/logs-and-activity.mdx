...
**durable Activity / Audit log** that survives restarts. The dashboard also
rolls request traffic up live:
...
- **`events`** — the structured `GET /api/journal` feed (hal0's internal
  ...
  `/api/journal/stream` SSE counterpart, which replays the last ~50
  matching entries before tailing live) accept the same server-side
  filters: `source` (exact match, or pass `source=slot` to match every
  `slot:*` source), `slot`, `level` (`info` / `warn` / `error`, exact
  match — unlike the "and higher severity" semantics of the `/api/logs`
  `level` filter above), `q` (substring match on the message), `since`
  (an id cursor, paged the same way as the Activity log below), and
