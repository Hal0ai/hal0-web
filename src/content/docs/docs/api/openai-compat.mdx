...
<Aside type="note">
  Authentication: hal0 binds `:8080` to all interfaces but **does
  not** require an API key by default — it's a home appliance, not a
  multi-tenant cloud. v0.2 adds Caddy + auth in front for remote
  access; until then, keep the port off the open internet.
</Aside>
