...
into VRAM, the backend has to pass its first health probe, and the
sentinel completion has to succeed. hal0's state machine encodes that
distinction explicitly, which is why the dashboard can show "warming —
12s elapsed" instead of "service is up" while the request still 503s.
