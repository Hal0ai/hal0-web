...
| `vulkan`    | gpu            | `ghcr.io/hal0ai/amd-strix-halo-toolboxes:vulkan-radv-server`         | off   | VULKAN:STD            |
| `flm`       | npu            | `ghcr.io/hal0ai/hal0-toolbox-flm:0.9.43`                             | off   | NPU:FLM               |
| `tts`       | cpu            | `ghcr.io/hal0ai/hal0-toolbox-kokoro:v1`                              | off   | TTS                   |
| `comfyui`   | img            | `docker.io/kyuz0/amd-strix-halo-comfyui@sha256:0066678ae9043f69…`   | off   | IMG:GEN               |
...
<Aside type="caution" title="Retired profile slugs">
The slugs `rocm` (and `rocm-mtp`) are retired. Existing slot TOMLs that reference
`rocm` are transparently remapped to `rocm-moe` by the loader; `rocm-mtp` has
been replaced by the per-slot `mtp` field.
</Aside>
