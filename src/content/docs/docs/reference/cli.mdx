...
description: "Complete reference for the hal0 CLI — top-level commands and every sub-app: slot, model, registry, memory, config, doctor, capabilities, agent, upstream, mcp, migrate."
sidebar:
  order: 10
---
...
- `--hardware` — `vulkan` | `rocm` | `cpu`. Auto-detected from
  ...
  `cpu→cpu`.
- `--port` / `-p` — host port; auto-assigned from the free range `8081–8200`
  when omitted.
...
| `memory enable` | Turn the memory subsystem on (sets `[memory].enabled = true`). | — |
| `memory disable` | Turn the memory subsystem off (sets `[memory].enabled = false`). | — |
...
| `memory bank list` | List Hindsight banks. | `--json` |
| `memory bank create <id>` | Create a Hindsight bank. | — |
| `memory bank delete <id>` | Delete a Hindsight bank. | `--force`/`-f` |
| `memory ops list` | List memory operations (retention, recall, etc.). | `--json` |
| `memory mm list` | List mental models. | `--json` |
| `memory recall <query>` | Run a recall query against the active provider. | `--max-tokens` |
| `memory migrate` | Migrate a legacy Cognee store to Hindsight banks (dry-run only). | `--dry-run` (default on), `--cognee-dir`, `--json` |

`memory enable`/`disable` replace the old `HAL0_MEMORY_ENABLED` env var
(removed in v0.9.8) — the whole subsystem (Hindsight engine, `/mcp/memory`,
`/api/memory/*`, the dashboard's Agent → Memory tab) is gated by
`[memory].enabled` in `hal0.toml`, defaulting to `true`. The provider is
built once at `create_app()`, so flipping it needs a `hal0-api` restart.
...
<Aside type="caution">
The memory subsystem defaults **on** in v0.9.8 (via `[memory].enabled = true`).
Graph extraction still defaults **off** (ADR-0014): small local models flake
on structured-output prompts, which would leave the graph view empty. See
...
| `agent install <name> --switch` | Atomically swap to a new agent (uninstalls the incumbent first). | `--switch` (required) |
...
## `hal0 upstream` — remote LLM providers

| Command | Purpose | Key options |
|---|---|---|
| `upstream list` | List configured upstream providers. | `--json` |
| `upstream show <name>` | Show detail for one upstream provider. | `--json` |
| `upstream create <name>` | Register a new upstream provider. | `--url`, `--api-key`, `--catalog` |
| `upstream update <name>` | Update an upstream provider's settings. | `--url`, `--api-key`, `--enabled`, `--model-filters` |
| `upstream delete <name>` | Remove an upstream provider. | `--force`/`-f` |
| `upstream test <name>` | Probe an upstream's reachability and auth status. | — |
| `upstream credentials set <name>` | Write credentials for an upstream provider. | `--api-key` (chains create + set) |
| `upstream advertise <name> <state>` | Toggle `/v1/models` catalog visibility (`on` / `off`). | — |

`upstream advertise` flips the `advertise_models` flag live (no `hal0-api`
restart). Advertising controls catalog visibility only — dispatch/routing to
the upstream by explicit id is unaffected. The change takes effect on the next
`/v1/models` request.
