...
<Aside type="note">
When Hermes or another bundled agent calls the MCP server, it presents a Bearer
token in the HTTP transport headers. The server stamps the caller identity on
every audit row and re-attaches the same token on the internal REST call — an
agent can only do through MCP what its token already permits through REST. This
is distinct from the external API surface, which has no built-in auth (ADR-0012).
Each tool call is written to the audit log.
</Aside>
