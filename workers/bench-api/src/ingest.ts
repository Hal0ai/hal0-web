...
export async function ingestHandler(req: Request, env: Env): Promise<Response> {
  ...
  const existing = await env.DB.prepare("SELECT id FROM bundles WHERE id = ?")
    ...
    .first<{ id: string }>();
  if (existing) {
    return json({ bundle_id: bundle.bundleId, url: BENCHMARKS_URL, records: 0, duplicate: true });
  }

  if (bundle.records.length > 0) {
    const conditions = bundle.records.map(() => "(cell_key = ? AND run_id = ?)").join(" OR ");
    const params = bundle.records.flatMap((r) => [r.cellKey, r.runId]);
    ...
    if (collision && collision.bundle_id !== bundle.bundleId) {
      return errors(
